...
For confirmed multi-page Lanhu scopes, page fan-out is only an evidence-fidelity strategy. The selected role-and-format analyst is called once per page and writes a complete page package under `.lanhu/MM-DD-<requirement-name>/pages/<page-slug>/`; frontend HTML page packages contain their own `index.md`, `index.html`, and `prototype/index.html`. The aggregate package root keeps only a global `index.md` for page package listing, reading order, cross-page relationships, Mermaid relationship graph when useful, aggregated scope summary, and confirmation status. It must not contain a root-level final HTML PRD generated from page summaries. Compact page metadata, `.yaml`, or summary Markdown are not PRD sources and must not be expanded into final HTML by the main session.
