...
- Let implementation and review consume plan `Referenced Project Wiki` plus task-scoped `.wiki-context.json` renders via `--task-id`, without re-invoking `wiki-researcher` during execution/review, while sourceOfTruth enforcement is handled by deterministic changed-path lint instead of task-scoped source-truth sidecars
- Patch Superpowers `finishing-a-development-branch` with a durable-knowledge gate at the top of its process (leading with, and not displacing, the native Step 1 test verification) so the `update-wiki` keep-or-skip determination is made by running the skill — not pre-decided inline — before a finishing option removes the worktree and its candidates sidecar; merge-back uses native base-branch detection and `using-git-worktrees` is left unpatched
...
4. During planning, read only candidate section full text; `wiki-researcher` writes its JSON selection to `<plan-stem>.wiki-selection.json` at plan phase and returns a compact summary (keeping the full selection out of the main agent's context); the planning agent turns that file into the sidecar mechanically with `wiki_context_render.py <sidecar> --scaffold <selection>` (schemaVersion 4, one bounded `documentContext` per wiki page, nested selected sections, auto reread for hard sections), then edits only the semantic routing
5. After final tasks stabilize, assign routing on each section's `destination` (`kind` + `reason`, plus `tasks` for task-bound) in a single edit pass, then finalize mechanically with `wiki_context_render.py <sidecar> --finalize --plan-path` — one write that scaffolds the `taskWikiRefs` roster, stamps `wiki/source task fingerprint` (never hand-write it), and validates `--execution-ready`; then render execution context with `--task-id`
6. During implementation and review, use plan `Referenced Project Wiki` and render selected task constraints from linked `.wiki-context.json`; hard-constraint rereads are materialized by a single fixed fetcher (`wiki_materialize_task.py`) that pulls local wiki sections from disk and `source: github_mcp` sections via the shared-wiki MCP `read-sections` CLI, injecting document context plus the selected section body only and failing closed on shared-wiki rebinding/revision drift
7. Do not call `wiki-researcher`, dispatch wiki-selection subagents, or manually reread wiki roots during implementation/review; missing or insufficient execution context returns to planning
8. Avoid full-tree wiki loading unless explicitly requested, and do not inject sibling sections or full pages just to recover section context
...
Implementation and review consume this plan section and linked sidecar context instead of reselecting wiki pages from scratch. The sidecar is schemaVersion 4 JSON with page-rooted `wikiPages`, one bounded `documentContext` from `<stem>.index.md` per page, nested selected `sections`, and categorized implementation/test/review/general constraints. The planning agent generates the sidecar from the `wiki-researcher` selection with `wiki_context_render.py <sidecar> --scaffold <selection>` and edits only semantic routing; after final task stabilization the agent assigns `taskRouting` and each section's `destination` routing (`kind` + `reason` + `tasks` for task-bound) in a single edit pass, then `--finalize` mechanically scaffolds the `taskWikiRefs` roster, stamps `wiki/source task fingerprint`, and validates `--execution-ready` in one write (the standalone `--scaffold-tasks` / `--bind-fingerprints` subcommands remain for the execution-side rebind-on-drift path); execution preflights with `--fingerprint-preflight`, then renders selected task constraint blocks with `--task-id` instead of task-string filtering. Forced hard-constraint rereads use the task-scoped page context with the selected section body, not sibling sections or whole pages.
