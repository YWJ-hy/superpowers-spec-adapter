...
For URL-rooted selection that resolves to multiple Lanhu target pages, page fan-out is only an evidence-fidelity strategy. The selected role-and-format analyst is called once per selected page and writes a complete page package under `.lanhu/MM-DD-<requirement-name>/pages/<page-slug>/`; frontend HTML page packages contain their own `index.md`, `index.html`, and `prototype/index.html`. The aggregate package root keeps only a global `index.md` for page package listing, reading order, cross-page relationships, Mermaid relationship graph when useful, the root tree selection summary, `selectedTargetPages`, aggregated scope summary, and confirmation status. It must not contain a root-level final HTML PRD generated from page summaries. Compact page metadata, `.yaml`, or summary Markdown are not PRD sources and must not be expanded into final HTML by the main session. After multiple per-page PRD packages are generated and gates are clear, the main session may ask whether the user wants optional cross-package synthesis, such as summarization, relationship extraction, shared business goal identification, page-flow/dependency mapping, or judging whether pages belong to one business requirement; synthesis happens only after per-page PRDs and must not replace them.

For Lanhu URLs with an explicit `pageId`, the main session first reads only the URL current page and descendant page tree metadata, then selects target pages from that tree based on the user's description. The user can explicitly exclude the current/root page and name child pages to analyze. If a named page is outside the URL-rooted tree, the main session asks for a corrected page name or different root URL instead of reading broadly. If matching is ambiguous, it asks the user to choose from tree candidates by page name/path/pageId. Full scoped evidence is read only by the page analyst for its own selected page, with descendants excluded by default; child pages become PRD targets only when selected and dispatched separately.
