...
# Source-of-truth dispatch/binding detail is deferred to an on-demand reference contract so
# unconfigured plans never load it; the moved detail must survive in the reference.
SOURCE_TRUTH_REF="${TARGET_INPUT}/contracts/source-truth-verification.md"
if [[ ! -f "$SOURCE_TRUTH_REF" ]]; then
  printf 'Expected installed source-truth verification reference contract\n' >&2
  exit 1
fi
for required in \
...
do
  if ! grep -Fq -- "$required" "$SOURCE_TRUTH_REF"; then
    printf 'Expected source-truth verification reference to retain deferred detail: %s\n' "$required" >&2
    exit 1
  fi
done
...
# R8 regression guard: the execution renderer consumes only the lightweight constraints sidecar; it
# must never machine-read the planning/audit report (that would defeat the bounded-envelope split).
SOURCE_TRUTH_RENDER="${TARGET_INPUT}/scripts/source_truth_render.py"
if grep -Fq -- 'report' "$SOURCE_TRUTH_RENDER"; then
