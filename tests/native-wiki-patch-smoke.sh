...
for required in 'wikiRoots:' '.shared-superpowers/wiki' 'root-prefixed `displayPath`' 'sharedWikiSource: auto' 'github_mcp' 'wikiPath' 'revision' 'logical display path' 'shared-wiki MCP' 'selectionOutputPath' 'Plan-phase return' 'disallowedTools: Task, Agent' 'Invoke subagents or Task/Agent tools'; do
...
for required in 'Adapter Task Context' 'wiki_context_render.py' '--task-id' '--fingerprint-preflight' '--execution-ready' '--role implementer' 'wiki_materialize_task.py' 'source: github_mcp' 'read-sections' 'Worktree first' 'superpowers:using-git-worktrees' 'GIT_DIR != GIT_COMMON' '## Assigned Task' '## Rendered Wiki Constraints for This Task' '## Hard Wiki Constraint Rereads' 'rebinding drift' 'do not call `wiki-researcher`' 'dispatch another wiki-selection subagent' 'manually reread wiki roots' 'return to planning to repair `Referenced Project Wiki`' 'Adapter Source-of-Truth Task Lint' 'source_truth_settings.py' '--render-prompt execution-reminder' '--lint-changed' '--changed-path' '--authorized-truth-edit' 'truth/edit: never' 'truth/edit: ask' 'keep-or-skip determination about durable implementation knowledge' 'and bypass the skill' 'candidate absence is not conclusive'; do
...
for required in 'Adapter Task Context' 'wiki_context_render.py' '--task-id' '--fingerprint-preflight' '--execution-ready' '--role implementer' '--role reviewer' 'wiki_materialize_task.py' 'source: github_mcp' 'read-sections' 'revision drift' 'Worktree first' 'superpowers:using-git-worktrees' 'GIT_DIR != GIT_COMMON' 'file-handoff model' 'task-brief' 'review-package' '-wiki.md' 'git rev-parse --show-toplevel)/.superpowers/sdd' '## Hard Wiki Constraint Rereads' 'rebinding drift' 'do not call `wiki-researcher`' 'dispatch another wiki-selection subagent' 'manually reread wiki roots' 'return `NEEDS_CONTEXT`/`BLOCKED` to the controller' 'Adapter Source-of-Truth Task Lint' 'source_truth_settings.py' '--render-prompt execution-reminder' '--lint-changed' '--changed-path' '--authorized-truth-edit' 'truth/edit: never' 'truth/edit: ask' 'keep-or-skip determination about durable implementation knowledge' 'and bypass the skill' 'candidate absence is not conclusive'; do
