...
只有在用户入口明确后，再实现或调整 `overlays/scripts/*.py`。涉及 spec 内容判断的 command / skill 应优先由 agent 主导；Python 只做 inventory、copy、validate、refresh 等机械操作，不应独立判断 durable knowledge、target ownership 或 contract 内容。
