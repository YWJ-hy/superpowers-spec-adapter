...
但这只能说明脚本本身可执行，不能说明用户在 Claude Code 中可以正确使用 `update-spec` skill 或 native skill 集成路径。
...
还应确认安装后 `update-spec` skill 会引导 agent 先判断是否存在 durable knowledge，再读取 indexed specs、做语义去重、判断目标归属、直接编辑 leaf spec 并刷新索引。
...
只有在用户入口明确后，再实现或调整 `overlays/scripts/*.py`。涉及 spec 内容判断的 command / skill 应优先由 agent 主导；Python 只做 inventory、copy、validate、refresh 等机械操作，不应独立判断 durable knowledge、target ownership 或 contract 内容。新增 bug 复盘能力时，bug 修复过程仍由 Superpowers `systematic-debugging` 负责，复盘由 `break-loop` 负责，spec 写入仍由 `update-spec` 负责。
