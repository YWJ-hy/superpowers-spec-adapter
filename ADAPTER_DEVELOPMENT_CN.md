...
3. 相关 overlay skill 或 agent，例如：
   ...
   - `overlays/agents/source-of-truth-verifier.md`，这是 planning 阶段 draft plan 后、final plan review 前的可选真实源校验入口；仅在 `sourceOfTruth` 已配置或用户显式要求时校验配置化真实源假设并输出 report/constraints sidecar，不实现、不改 plan、不修改真实源
...
`source-of-truth-verifier` 是独立 agent，不是 `plan-document-reviewer` 的扩展。`writing-plans` 必须先写出具体 draft plan，并先用 `source_truth_settings.py --show-policy` 检查 `.superpowers/settings.json` 的 `sourceOfTruth`；只有 `sourceOfTruth` 已配置或用户显式要求真实源校验时，才让 verifier 校验 plan 中的接口、字段、类型、schema、权限、设计 token 和项目规则假设，然后由 planning agent 修订 final plan，最后交给原生 plan review。未配置且未显式要求时跳过 verifier 和 sidecar。
...
5. 如果修改 planning wiki 披露流程，确认 plan 的 `Referenced Project Wiki` 是轻量入口，并正确链接 `docs/superpowers/plans/<plan-stem>.wiki-context.json`；planning agent 从 `writing-plans` 常驻块内联的 schemaVersion 3 骨架写 sidecar，仅在无法从内联骨架确定结构或校验报结构错误时才回退查阅安装后的 `contracts/wiki-context-v3.example.jsonc`，且不应读取 `wiki_context_render.py` 源码来反推格式；sidecar 应使用 schemaVersion 3 page-rooted `wikiPages`，每个 page 只保留一份有界 `documentContext`，sections 保留 `relevanceTo`、分类约束、hard constraint、reread 和 anchors；final task 稳定后再写入 `taskRouting`、`taskWikiRefs`、`globalWikiRefs` 和 `destination`，planning 阶段必须用 plugin-root `wiki_context_render.py --bind-fingerprints --strict --execution-ready --plan-path <plan>` 从 plan 任务文本机械 stamp `taskFingerprint` 并校验 sidecar 执行就绪（不得手写或复制指纹；copied placeholder 能过结构校验但会在执行期 preflight 失败），并把 selected wiki constraints 吸收到 plan/task 文本中；进入执行前先做一次 `--fingerprint-preflight --execution-ready --strict --plan-path <plan>`，执行阶段必须通过 plugin-root `wiki_context_render.py --task-id <task-id> --role <role> --strict --execution-ready` 渲染 task-scoped constraints，不按 task string 过滤 wiki 约束，并通过 `--task-id <task-id> --reread-list --execution-ready` 只重读当前 task 选中的 hard section document context + section body，不注入 sibling sections 或整页正文。
6. 如果修改 source-truth 流程，确认 `writing-plans` 是 draft plan → 检查 `sourceOfTruth` 配置/显式请求 → 仅启用时运行 `source-of-truth-verifier` → final task 稳定后把 `constraintSets` 路由到 `taskConstraintRefs` / `globalConstraintRefs` 并用 `source_truth_render.py --bind-fingerprints` 机械 stamp `taskFingerprint` → final plan → `plan-document-reviewer` 顺序；未启用时不生成 not-configured sidecar。`writing-plans` 常驻块只保留条件触发、`source_truth_settings.py --show-policy` 探针、`not_configured` skip 规则，以及指向安装后 `contracts/source-truth-verification.md` 的强制引用；verifier dispatch 输入、有界 envelope 契约、两阶段绑定和 `source_truth_render.py --bind-fingerprints --strict --execution-ready` 校验细节都迁到该 reference contract，未配置时不加载。执行阶段只在 plan 链接 `.source-truth-constraints.json` 时通过 plugin-root `source_truth_render.py --fingerprint-preflight` 和 `source_truth_render.py --task-id <task-id>` 消费约束，并把 renderer stdout 带 task/role/source sidecar 边界标注直接注入 prompt，不读取完整 `.source-truth-report.json`，不写 rendered Markdown 上下文文件，也不使用 legacy `appliesTo` 或 task string 路由。执行渲染器只消费 constraints sidecar，永不机读 report（`source_truth_render.py` 不得出现 `report` 引用）。还要运行 `tests/source-truth-settings-smoke.sh`、`tests/source-truth-render-smoke.sh` 和 native patch smoke。还要确认 `source-of-truth-verifier` 自己写出 report/constraints 并只返回有界 envelope，主 agent 不把完整 report 读入上下文；在安装后集成路径验证 subagent Write 成功，并保留 Write 失败回退。
