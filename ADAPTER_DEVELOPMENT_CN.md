...
3. 相关 overlay command、skill 或 agent，例如：
   ...
   - `overlays/commands/lanhu-requirements.md`，这是可选蓝湖需求文档入口；无设计稿时写入 `.lanhu/MM-DD-需求命名.md`，有设计稿时写入 `.lanhu/MM-DD-需求命名/prd.md` 和 `.lanhu/MM-DD-需求命名/design/`，然后等待用户确认
   - `overlays/agents/wiki-researcher.md`，这是正常流程的 wiki 选择入口
   - `overlays/agents/lanhu-requirements-analyst.md`，这是可选蓝湖需求清洗入口，不做实现分析
...
Lanhu 集成必须保持可选：不能要求用户安装 lanhu-mcp 才能使用 adapter；Lanhu 产物只能作为用户确认的需求输入写入用户项目根目录。无设计稿时写 `.lanhu/MM-DD-需求命名.md`；有设计稿、原型、视觉标注或切图资源等设计信息时写 `.lanhu/MM-DD-需求命名/prd.md` 和 `.lanhu/MM-DD-需求命名/design/`。用户确认后再进入 Superpowers `brainstorming`。`prd.md` 或单文件 `.md` 是需求入口，`design/` 只保存设计事实、可见 UI 内容、设计备注、资源索引或可精确获取的 Lanhu 资源。Lanhu 产物不得包含测试点、验收标准、前端组件拆分、后端接口推测、数据库影响、实现方案或代码文件影响，也不得写入 `.superpowers/wiki/`、`Referenced Project Wiki` 或 plan sidecar。
