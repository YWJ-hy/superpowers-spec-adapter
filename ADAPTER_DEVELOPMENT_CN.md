...
Python 脚本是执行层，command、skill、hook 才是用户实际接触到的产品入口。
...
| 层 | 代表文件 | 责任 | 测试关注点 |
|---|---|---|---|
| 用户入口层 | `overlays/commands/*.md`、`overlays/skills/*/SKILL.md` | 定义 Claude Code 中用户如何调用能力 | 文案是否引导 agent 走正确流程 |
| Hook 层 | `overlays/hooks/*`、`lib/hook_patch.py` | 会话启动时注入轻量状态 | 安装后 hook 是否真的生效 |
| 执行层 | `overlays/scripts/*.py` | 执行 spec、sidecar、gate 等文件操作 | 脚本行为是否正确、可组合 |
| 安装层 | `install.sh`、`manage.sh`、`verify.sh`、`release-check.sh` | 把 overlay 写入 Superpowers 插件目录 | 安装产物和 patch 是否完整 |
...
但这只能说明脚本本身可执行，不能说明用户在 Claude Code 中可以正确使用 `/update-spec` 或 `/check-workflow`。
...
3. 对应 command 文档仍会引导 agent 调用正确执行层
4. 在目标项目中能通过 Superpowers command / skill 完成用户场景
...
3. 在 Claude Code 中触发对应 command，例如：
...
3. 确认 SessionStart 注入的是轻量摘要树和 plan sidecar 状态，而不是全文 spec。
4. 运行：
...
- 底层脚本行为正确
- overlay command / skill 能正确引导用户路径
