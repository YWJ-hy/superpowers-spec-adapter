...
3. 相关 overlay skill 或 agent，例如：
   ...
   - `overlays/agents/source-of-truth-verifier.md`，这是 planning 阶段 draft plan 后、final plan review 前的真实源校验入口；它只校验配置化真实源假设并输出 report/constraints sidecar，不实现、不改 plan、不修改真实源
...
只有在用户入口明确后，再实现或调整 `overlays/scripts/*.py`。涉及 wiki 内容判断的 skill 应优先由 agent 主导；Python 只做 inventory、copy、validate、refresh、过大页面统计等机械操作，不应独立判断 durable knowledge、target ownership、拆分边界或 contract 内容。
...
### Source-of-truth verifier 边界

`source-of-truth-verifier` 是独立 agent，不是 `plan-document-reviewer` 的扩展。`writing-plans` 必须先写出具体 draft plan，再让 verifier 按 `.superpowers/settings.json` 的 `sourceOfTruth` 配置校验 plan 中的接口、字段、类型、schema、权限、设计 token 和项目规则假设，然后由 planning agent 修订 final plan，最后交给原生 plan review。

开发时必须保持这些边界：

- `sourceOfTruth.heuristics` 默认关闭；未显式开启时，不按路径名猜真实源。
- 真实源只来自配置的 `sources`；已有接口调用、mocks、fixtures、wrapper、组件使用示例不能自动视为 truth。
...
Lanhu 包必须保留蓝湖原始需求中的明确事实；如果固定模板主题无法承接某条源事实，允许 analyst 创建按源需求内容命名的具体源事实主题，例如“计费规则源事实”“消息通知源事实”“导入导出源事实”，但不得丢失事实、弱化事实、合并成不可追溯摘要或使用“AI 自定源事实主题”“AI 自定业务源事实主题”“其他/杂项”等泛化兜底标题。frontend 只有一种输出形态：`.lanhu/MM-DD-需求名称/index.md` + `role-prd/prd.md` + 可选 `role-prd/design/index.html` / `role-prd/design/assets/`；`role-prd/prd.md` 不固定主题目录，按原始资料选择最清晰组织方式。Lanhu 包不得输出最终验收标准、Given / When / Then、测试点、测试用例、技术测试方案、前端组件拆分、后端接口推测、接口字段设计、数据库字段设计、数据库影响、实现方案、代码文件影响、前后端边界推断、异常/风险推断、独立证据映射表或 Superpowers plan tasks；缺少这些技术字段名或映射信息不得作为 Lanhu 阻塞确认点。

Deprecated `lanhu.frontend.output.format` 不再决定前端输出；如目标项目仍配置该字段，`lanhu_settings.py` 只返回忽略 warning。frontend 始终由 `lanhu-frontend-requirements-analyst` 写统一 `role-prd/` 包：`role-prd/prd.md` 作为规则、约束、边界和待确认问题主文档；当存在设计稿或需要交互 demo 时，可写 `role-prd/design/index.html`，用于 1:1 映射页面结构、控件关系、状态和交互路径，但不追求像素级视觉、生产级响应式、真实接口或复杂脚本。HTML demo 已清楚呈现的布局、控件类型和点击路径，不应在 `prd.md` 中长篇重复。后端角色必须始终 Markdown-only。
...
6. 如果修改 source-truth 流程，确认 `writing-plans` 是 draft plan → `source-of-truth-verifier` → final task 稳定后绑定 `constraintSets` 到 `taskConstraintRefs` / `globalConstraintRefs` / `taskFingerprint` → final plan → `plan-document-reviewer` 顺序；执行阶段只通过 plugin-root `source_truth_render.py --fingerprint-preflight` 和 `source_truth_render.py --task-id <task-id>` 消费 `.source-truth-constraints.json`，并把 renderer stdout 带 task/role/source sidecar 边界标注直接注入 prompt，不读取完整 `.source-truth-report.json`，不写 rendered Markdown 上下文文件，也不使用 legacy `appliesTo` 或 task string 路由。还要运行 `tests/source-truth-settings-smoke.sh`、`tests/source-truth-render-smoke.sh`、native patch smoke 和 Multica bootstrap/live/runtime smoke。
...
bash tests/source-truth-render-smoke.sh
