...
PLAN_REVIEW_BLOCK = '\n'.join(
    [
        REVIEW_START,
        '## Adapter Source-of-Truth Verification Review',
        '',
        'Generated by superpower-adapter.',
        '',
        'If the final plan contains a `Source-of-Truth Verification` section, review whether the plan correctly consumed the `source-of-truth-verifier` result. Do not redo the verifier\'s source-truth investigation; check the final plan behavior and execution handoff only.',
        '',
        'Verify that:',
        '',
        '- `status: blocked` plans do not offer execution handoff or implementation tasks that work around the block.',
        '- `needs_revision` findings have been converted into concrete plan changes, explicit user questions, or clearly marked unresolved blockers.',
        '- The plan does not ask implementers to edit configured `role: truth` sources with `edit: never`, such as generated clients, schemas, permissions, design tokens, or other protected contracts.',
        '- `role: truth` with `edit: ask` conflicts are not silently included as implementation work without explicit user authorization.',
        '- Relevant lightweight constraints are represented through `docs/superpowers/plans/<plan-stem>.source-truth-constraints.json` and task instructions, not by pasting the full report into the plan.',
        '- The full `*.source-truth-report.json` is treated as planning/audit context only and is not required as normal execution context.',
        '',
        'If the plan has no `Source-of-Truth Verification` section, do not fail solely for its absence; source-truth may be unconfigured or this may be an older plan. Judge the rest of the plan normally.',
        REVIEW_END,
    ]
)
...
def strip_plan_review_block(text: str) -> tuple[str, bool]:
    start = text.find(REVIEW_START)
    if start == -1:
        return text, False
    end = text.find(REVIEW_END, start)
    if end == -1:
        raise ValueError('malformed adapter plan-review source-truth marker: missing end marker')
    end += len(REVIEW_END)
    if end < len(text) and text[end:end + 1] == '\n':
        end += 1
    if start > 0 and text[start - 1:start] == '\n':
...
def find_plan_review_insert(text: str) -> int:
    if PLAN_REVIEW_EXPECTED_TEXT not in text:
        raise ValueError(f'expected prompt identity text not found: {PLAN_REVIEW_EXPECTED_TEXT}')
    insert_at = text.find(PLAN_REVIEW_INSERT_BEFORE)
    if insert_at == -1:
        raise ValueError(f'could not find insertion heading: {PLAN_REVIEW_INSERT_BEFORE}')
    if insert_at > 0 and text[insert_at - 1:insert_at] == '\n':
        insert_at -= 1
    return insert_at


def apply_plan_review_patch(path: Path) -> tuple[bool, str]:
    text = path.read_text(encoding='utf-8')
    text, removed = strip_plan_review_block(text)
    insert_at = find_plan_review_insert(text)
    updated = text[:insert_at] + '\n\n' + PLAN_REVIEW_BLOCK + '\n\n' + text[insert_at:].lstrip('\n')
    if updated != path.read_text(encoding='utf-8'):
        path.write_text(updated, encoding='utf-8')
        return True, 'Configured plan-document-reviewer source-truth review guidance'
    return removed, 'Plan-document-reviewer source-truth review guidance already satisfied'


def remove_plan_review_patch(path: Path) -> bool:
    if not path.is_file():
        return False
    text = path.read_text(encoding='utf-8')
    updated, removed = strip_plan_review_block(text)
    if removed:
        path.write_text(updated, encoding='utf-8')
    return removed


def verify_plan_review_patch(path: Path) -> None:
    if not path.is_file():
        raise ValueError('target file is missing')
    text = path.read_text(encoding='utf-8')
    if PLAN_REVIEW_BLOCK not in text:
        raise ValueError('plan-document-reviewer source-truth review guidance was not applied')
...
def main() -> int:
    ...
    plan_review_path = target / PLAN_REVIEW_RELATIVE_PATH
    try:
        if mode == 'install':
            if not plan_review_path.is_file():
                raise ValueError('target file is missing')
            spec_changed, message = apply_plan_review_patch(plan_review_path)
            changed = changed or spec_changed
            print(message)
        elif mode == 'uninstall':
            changed = remove_plan_review_patch(plan_review_path) or changed
        else:
            verify_plan_review_patch(plan_review_path)
    except ValueError as exc:
        failures.append(PatchFailure('plan-document-reviewer-source-truth', plan_review_path, str(exc)))
