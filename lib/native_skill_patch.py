...
**Sequence gate — do not emit any candidate approach until you have called `wiki-researcher` (phase: brainstorm).** This does NOT move or replace the native project-exploration step, and `wiki-researcher` must not run before it. The order is fixed: (1) explore project context exactly as this skill's process directs; (2) only after that exploration, and before proposing approaches, call `wiki-researcher` with `phase: brainstorm` (it consumes what the exploration surfaced as its `task`/`focus`); (3) then propose. If you arrive at drafting approaches without having called it, stop and call it now. The only exception is when no project/shared wiki applies — none exists, MCP is unavailable, or root indexes are missing — in which case record that one-line N/A reason and continue. Having read a lot of code is not a reason to skip this step.
...
After completing implementation, and before handing off to `finishing-a-development-branch` or otherwise removing/exiting the worktree, run the `update-wiki` skill to make the keep-or-skip determination about durable implementation knowledge for this work, consuming any `docs/superpowers/plans/<plan-stem>.wiki-candidates.jsonl` sidecar as candidate input. Reach that determination by actually invoking the skill so its judgment framework applies — do not pre-decide in the main loop that nothing is durable and bypass the skill, and do not treat an empty or absent candidates sidecar as proof there is nothing to record: candidate absence is not conclusive, because the skill also weighs the implemented changes and conversation context, not just the sidecar. Skipping is a valid outcome, but it must be the skill's own conclusion, stated with an explicit reason, not a reason to avoid invoking it. The sidecar lives in the working tree and is deleted when the worktree is removed, so consume it here — inside the still-live worktree — not later. Do not defer this consumption to the post-merge `update-wiki` reminder: that reminder is only a backstop for when this step was skipped, and once a merge completes the worktree (and the sidecar) is usually already gone, leaving update-wiki to fall back to the merged diff and conversation context. Do not force a spec or wiki edit when the skill concludes there is nothing durable to record.
''',
    ),
    PatchSpec(
...
- Otherwise, run the `update-wiki` skill now, consuming the candidates sidecar if present. Reach the determination by actually invoking the skill so its judgment framework applies — do not pre-decide in the main loop that nothing is durable and bypass the skill, and do not treat an empty/absent candidates sidecar (or subagents that surfaced no candidates) as proof there is nothing to record: candidate absence is not conclusive, because the skill also weighs the implemented changes and conversation context, not just the sidecar. Skipping is a valid outcome, but it must be the skill's own conclusion, stated with an explicit reason.
