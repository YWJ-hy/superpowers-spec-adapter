...
Then assign routing by editing each selected section's `destination` in the generated sidecar: finalize `destination.kind` (`planning-only` for soft context the task text already embodies — not injected at execution/review, never for `hardConstraint`/`direct`; `global` for rules every task and reviewer needs; else `task-bound`), write a one-line `destination.reason`, and for every `task-bound` section list the plan task ids it applies to in `destination.tasks` (for example `["T1", "T3"]`). There are no `globalWikiRefs` or per-task `wikiRefs` collections and no `sectionRef` to hand-write — the renderer derives each task's wiki scope straight from `destination.kind` and `destination.tasks`. Then flip `taskRouting.status` to `confirmed` with `selectedSectionsFrozen: true`. For a reviewer-only check, keep the section `task-bound` and put it in the `review` constraint category instead of re-stating it to the implementer. Do not hand-write or copy `taskFingerprint`; stamp it mechanically from the reviewed plan, which validates execution readiness and writes the sidecar in place:
