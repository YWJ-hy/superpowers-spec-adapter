...
subagent 不应重新从 `.superpowers/wiki/` 选择规范，除非主 agent 判断 plan 引用明显不足并回到 planning 修正。所有 implementer / reviewer / final reviewer 的派发只能由主 agent / controller 完成；subagent 是叶子节点，禁止继续派发 nested subagent 或调用 `Task` / `Agent` / `Workflow` 做二次委托。这个限制只约束 subagent 内部，不禁止主 agent 按 SDD 流程派发多个同级 sibling subagents；如果 subagent 需要更多上下文、拆分任务或升级模型，应返回给主 agent 处理。sourceOfTruth 不通过 sidecar 分发给 subagent；orchestrator 在每个任务完成前按真实 changed files 运行 lint，并让 reviewer 检查 findings 是否已处理。
