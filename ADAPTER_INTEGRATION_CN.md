...
分发 implementer / reviewer subagent 前，主 agent 应读取 plan 的 `Referenced Project Wiki`，用 plugin-root `wiki_context_render.py` 按分配任务分别渲染 implementer / reviewer 约束块，并把渲染结果**写到文件**（SDD handoff 目录 `git rev-parse --git-path sdd` 下的 `task-<id>-wiki.md` / `task-<id>-wiki-review.md`），在 subagent prompt 里只给文件路径让其 Read —— 对齐 6.0.0 原生 `task-brief` / `review-package` 的 file-handoff 模型，不把完整任务文本或渲染结果粘进 prompt（任务文本由原生 `task-brief` 负责）。hard constraint 原文回读同样从 task-scoped `--reread-list` 批量处理，并追加到同一任务 wiki 文件；batch 只是减少调用次数，不改变只读 selected sections、不读 sibling/full page、按原始列表顺序注入的语义。task id 使用 plan 的纯数字任务号（`### Task 1:`），与原生 `task-brief` 抽取一致。
