...
- `overlays/wiki-repo-ci/`：`export-wiki-skills` **默认**钉版本写入目标 wiki 仓库 `.github/workflows/rebuild-wiki-graph.yml` 的 GitHub Action 模板（首行 marker 占位，导出时替换）。它解决消费者侧更新的一个缺口：PR（含 shared-wiki MCP PR 路径）只带得动逐页 companion `*.index.md`，跨页根 `.graph.json`（section 边 + backlinks，正是 MCP `graph-neighbors` / 执行期 github_mcp 硬约束 1 跳 `depends-on` 闭包读的那份）进不了 patch，合入后会陈旧到维护者手动重建。该 Action 在 push 到默认分支时复用**同一份** vendored `wiki_migrate_helper.py --generate-indexes` 重建图并带 `[skip ci]` 回写（只在默认分支、no-diff 不提交，图生成器与维护者侧绝不分叉）。`export-wiki-skills --no-graph-ci` 跳过；再次 `--no-graph-ci` 导出会移除先前钉的 marker 文件，但不动手写的同名 workflow。
- `lib/`：adapter 自身的安装、manifest、hook 配置维护、native skill patch、目标 Superpowers 目录解析逻辑；`export_wiki_skills.py` 是 `export-wiki-skills` 的导出引擎（含 marker 防覆盖与脚本哈希 manifest，默认还导出 `overlays/wiki-repo-ci/` 的图重建 Action，`--no-graph-ci` 关闭并移除自己钉的那份）。
