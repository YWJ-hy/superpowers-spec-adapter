...
- section 之间可用 `[[page#section]]` wikilink 显式声明跨引用（页路径相对 wiki root，`[[#section]]` 指同页 section，`[[page]]` 指整页）。只有 `[[ ]]` 算知识边，普通 `(path#anchor)` 散链不算；本期边为无类型的 see-also。索引生成时机械派生 section 级图谱：每个 wiki root 写一份 `.graph.json`（节点 / 边 / backlinks / dangling），并在 `<stem>.index.md` 表格补「引用 / 被引用」两列。该图谱是 wiki 层产物，只供维护（`update-wiki`）与校验（`doctor`）消费，**不在执行期被追链**——执行仍只消费 plan 中已选中的 section（schemaVersion 4 路由不变）。
