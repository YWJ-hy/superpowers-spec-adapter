...
```text
描述需求 / 可选蓝湖链接
→ 如果使用蓝湖，先确认前端/后端角色；/lanhu-requirements 路由角色 analyst 直接生成 .lanhu/MM-DD-需求名称/ 需求包，默认 Markdown-only；如目标项目配置前端 html，可生成 index.html 完整 HTML PRD 主文档和 prototype/index.html 1:1 交互复刻原型；主会话只接收路径摘要和紧凑确认门禁，index.md 是入口和 PRD 关系权威来源
...
如果蓝湖链接带有明确 `pageId`，adapter 会在角色确认后固定调用 scoped Lanhu MCP 工具：先用 `lanhu_get_prd_page_scope` 获取目标页和可确认子页 metadata，再在用户确认子页范围后用 `lanhu_get_prd_scoped_evidence` 读取 `output_mode: evidence_only` 的证据。目标页始终纳入；子页只有用户确认后纳入；相邻页面、同文档其它模块、父级流程页、垃圾站 / 旧页面、导航关联页或 Lanhu AI 认为“相关”的页面不会进入 adapter 上下文。页面 scope 只决定证据范围，最终 PRD 数量仍由业务交付边界决定；需要多页、整条流程或整个原型时，用户应显式说明。
