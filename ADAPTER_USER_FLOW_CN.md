...
- 在 `writing-plans` 阶段正式选择相关项目 wiki 页面，由 `wiki-researcher` 在 plan 阶段自行把 JSON selection（结构见 `contracts/wiki-selection-v1.example.jsonc`）写入 `docs/superpowers/plans/<plan-stem>.wiki-selection.json` 并只回紧凑摘要（选中页/计数/caveats，不回灌整份 selection），主 agent 据此直接对该文件用 plugin-root `wiki_context_render.py <sidecar> --scaffold <selection> --strict --plan-path <plan>` 机械生成成形的 `.wiki-context.json` 骨架（自动补常量、`taskRouting`、每个 hardConstraint 的 `reread`、github_mcp 的顶层 `sharedWiki` 身份、默认 `destination.kind`），AI 只编辑每段 section 的语义路由 `destination`（`kind`、`reason`、task-bound 的 `tasks`）与 `taskRouting.status`，并要求 plan 写入轻量 `Referenced Project Wiki` 入口。JSON 以 wiki page 为根节点，每个 page 只保留一份来自伴随 `<stem>.index.md` 的有界 `documentContext`，选中的 sections 作为子节点并保留 implementation / test / review / general 分类约束；final task 稳定后，AI 一次性完成各 section 的 `destination` 路由编辑与 `taskRouting.status` 确认，再用 `wiki_context_render.py <sidecar> --finalize --strict --plan-path <plan>` 一步机械补 `taskWikiRefs` roster（`taskId`/`taskTitle`，保留已填 `taskFingerprint`）、从 plan 任务文本 stamp `wiki/source task fingerprint`、校验执行就绪并单次写盘（合并原 `--scaffold-tasks`+`--bind-fingerprints` 两次写为一次，被追踪的 sidecar 少回灌一次；不手写或复制指纹）。`--scaffold` 成功后消费并删除该 selection（只留 plan 与 `.wiki-context.json`，`--keep-selection` 可保留），报结构错误时保留浅层 selection、修复后重跑 `--scaffold`，仅在生成器不可用时才回退手写 `contracts/wiki-context-v4.example.jsonc`。
- 在执行阶段只消费 plan 中已经确认的 `Referenced Project Wiki` 和其链接的 `.wiki-context.json`。`writing-plans` 在 final task 稳定后才把 selected wiki sections 绑定到 task，执行 / SDD 只按 `--task-id <stable-task-id>` 渲染该 task 的 wiki 约束，并在进入执行前先做一次 fingerprint preflight；renderer stdout 会由主 agent 捕获，并带 `## Rendered Wiki Constraints for This Task` 等边界标注直接注入当前 task / subagent prompt，不作为 `.claude-*-wiki-task*-impl.md` 或 `.claude-*-source-task*-impl.md` 这类 rendered Markdown 上下文文件持久化。implementer / reviewer 不在执行期调用 `wiki-researcher`、不派发新的 wiki-selection subagent、不手动重读 wiki root、不重新选择 wiki；约束缺失或明显不足时返回 planning 修复 `Referenced Project Wiki` 和 sidecar。`wiki-researcher` 自身也是叶子选择器，禁止再派发子 agent，包括同名 `wiki-researcher`。如果 plan 在审核后被手工修改，先确认 selected wiki routing 仍适用于改动的 task，再重新运行 `wiki_context_render.py <sidecar> --bind-fingerprints` 刷新 `wiki/source task fingerprint`，不重写 plan。对 `hardConstraint: true` 的 section，执行阶段会通过 `--task-id <stable-task-id> --reread-list --execution-ready` 强制回读当前 task 对应的原始 wiki section 全文（通过 `<!-- wiki-section:xxx -->` 标记提取），并在 `## Hard Wiki Constraint Rereads` 边界下注入选中 section 全文（页面有界 document context 由 renderer 每页只渲染一次，reread 块不再重复它），确保约束不因摘要信息衰减或 section 脱离页面主语而被误用。渲染按角色过滤：section 若带 `roles`（只在发现卡片上出现，由 `--scaffold` 从卡片 marker 机械 stamp，取值是 `implement`/`review` 的严格子集），则 `--role implementer` 与 `--role reviewer` 各自只看到绑定本角色的 section——`render` 与 `reread` 都过滤，因此一张 `roles: review` 的审查类 skill 卡片只会注入 reviewer、绝不进 implementer prompt；普通 section 不带 `roles`，对两个角色一视同仁（沿用既有行为）。
- Wiki 文档使用 `<!-- wiki-section:section-id -->` / `<!-- /wiki-section:section-id -->` 标记包裹独立约束主题段落，每个叶子文档都必须有 `<stem>.index.md` 伴随索引；该索引包含文档级语义概览和 section 表格。`wiki-researcher` 通过读取 per-document index 快速判断文档和 section 相关性，未迁移到新格式的文档不参与 wiki-researcher 选择。用户可通过 `migrate-wiki` skill 将现有 wiki 迁移到 section-marker 格式。该 skill 入口提供两种模式:**模式 1「仅 section 化」**(现状);**模式 2「section 化 + 图谱梳理」**——对已 section 化的 wiki 进一步设节点 `type:` 并补 `[[page#section]]` 边,走 `wiki.updateAuthorization` 授权、分批「提议→确认」后重生成 `.graph.json`/索引并 lint(细节在按需 companion `references/graph-enrichment.md`,模式 1 不加载)。
- section 之间可用 `[[page#section]]` wikilink 显式声明跨引用（页路径相对 wiki root，`[[#section]]` 指同页 section，`[[page]]` 指整页）。只有 `[[ ]]` 算知识边，普通 `(path#anchor)` 散链不算；裸 `[[ ]]` 是 `see-also`，typed 边用行内前缀 `[[type: page#section]]`（冒号后需空格），类型限 `see-also` / `depends-on` / `supersedes` / `contradicts`，未知前缀会被 lint 报 unknown edge type。
- 每个叶子页在 frontmatter 声明节点 `type:`——`constraint`（默认，可执行编码约束）/ `domain`（实现须遵循的稳定领域事实）/ `decision`（ADR 式决策，常带 `supersedes`/`contradicts` 边）/ `guide`（横切清单）。类型进 `.graph.json` 的 `pageTypes` 和 `<stem>.index.md` 头部 `> Type:`；未知值由 `wiki_update_check.py` 报警告。`wiki-researcher` 据类型聚焦选择（实现/调试先 `constraint`/`domain`，规划/评审再加 `decision`/`guide`），`update-wiki` 建新页时设对应类型——决策落 `decision` 页、领域事实落 `domain` 页。wiki 由此从"纯编码约束"扩成涵盖领域知识与重大决策的项目记忆层。索引生成时机械派生 section 级图谱：每个 wiki root 写一份 `.graph.json`（节点 / 边 / backlinks / dangling），`<stem>.index.md` 只渲染 section 名 / 描述 / 约束强度（wiki-researcher 判定 section 相关性所需），**不渲染知识图谱关系**——关系只活在 `.graph.json`，由 `wiki_graph_neighbors.py` / shared-wiki MCP 机械查询，不从索引表格解析。该图谱是 wiki 层产物，供维护（`update-wiki`）、校验（`doctor`）与规划期邻居发现消费。`wiki-researcher` 在规划期做 1 跳邻居发现时**不读整张 `.graph.json`**，而是用**有界查询**只取候选 `page#section` 节点的邻居切片：本地走 `wiki_graph_neighbors.py --node ...`，github_mcp 走 `shared_wiki_graph_neighbors({nodes})`；两者返回带 `type` 和 `indexed` 标志的 out/in 边，输出只随候选数增长、与 wiki 规模无关。执行期只追一种边：**hard-constraint** section 的 `depends-on` 边由 `wiki_context_render.py --reread-list` **1 跳闭包**（目标 section 拉进该 task 的 reread，有界——不传递追目标自己的 depends-on，仅本地 project section）；`see-also`/`supersedes`/`contradicts` 执行期一律不追。除这条有界 depends-on 闭包外，执行仍只消费 plan 中已选中的 section（schemaVersion 4 路由不变）。
...
- `update-wiki` 写入前读取目标 root 的 settings：`.superpowers/settings.json` 控制 project wiki，`.shared-superpowers/settings.json` 控制 shared wiki；默认更新已有页面跳过授权，创建新 wiki 文档询问用户授权；写入 shared wiki 前必须把内容中性化，不能保留当前系统特有标识。如团队使用 GitHub-backed shared-wiki MCP，则 shared wiki 写入通过 MCP validate patch + branch + PR，不直接改本地 shared wiki。`update-wiki` 是薄路由 `SKILL.md` + 按需 companion（`references/targeting.md` 选址授权、`references/content-templates.md` 模板清单、`references/shared-wiki.md` 中性化与 MCP PR、`references/graph-maintenance.md` section 标记与 `[[ ]]` 边及 backlink 感知编辑）；常见的「无 durable knowledge 跳过」路径只载薄路由器，改既有 section 前会读 `.graph.json` 的 backlinks 评估改动是否影响引用方。
- 安装 `break-loop` skill，用于 Superpowers `systematic-debugging` 修复并验证 bug 后做深度复盘，并在有长期价值时把候选交给 `update-wiki`。
- 安装 `scaffold-practice-skill` skill，把可复用的工程实践（管理页布局、微应用 host/child 文件结构、固定审查流程等）固化为 `.claude/skills/<name>/` 的**分层技能包**：唯一固定的是薄路由 `SKILL.md`，其余 `implement.md`/`review.md`/`rules.md`/`scripts/` 等是按需加载的开放集合（目的是渐进披露、省 token，并保证脱离 Superpowers 也能独立使用）。它也支持把现有单体 skill **非破坏式**转换为该结构（搬运全部附属文件、报告未覆盖内容、用户确认后才替换原件），并在项目 wiki `guides/skills.md` 机械登记**发现卡片**（含 companion 索引与索引链路修复），让 `wiki-researcher` 在计划阶段选中并绑定「必须使用 skill X」。`register-card --roles` 决定卡片绑定哪些执行角色：默认绑 implement+review（多数「怎么做」的实践，reviewer 也据其规范核对）；**审查类 skill** 用 `--roles review` 只绑 reviewer（审查清单永不注入 implementer），**纯生成类** 用 `--roles implement`。角色绑定写在卡片 marker 上，由执行层 render/reread 机械按角色过滤，不在 `SKILL.md` 内表达。关系是单向的：wiki 关联 skill，skill 不反向硬编码 wiki 路径。
...
│   └── update-wiki/
│       ├── SKILL.md            # 薄路由器
│       └── references/         # 按需加载 companion
│           ├── targeting.md
│           ├── content-templates.md
│           ├── shared-wiki.md
│           └── graph-maintenance.md
└── scripts/
    └── adapter 执行脚本
```

`wiki-progressive-disclosure` skill 已移除；正常 `brainstorming` 和 `writing-plans` 流程统一由 `wiki-researcher` 直接完成 wiki 选择，并通过严格 schemaVersion 4 `.wiki-context.json` 进入执行期。
...
- `executing-plans`：执行前读取 plan 中的 `Referenced Project Wiki` 和链接的 `.wiki-context.json`，先做一次 plugin-root `wiki_context_render.py <sidecar> --fingerprint-preflight --strict --execution-ready --plan-path <plan>`，再用 plugin-root `wiki_context_render.py --task-id <task-id> --role implementer --strict --execution-ready` 渲染 task-scoped 约束；renderer stdout 直接带 task/role/source sidecar 边界标注注入当前任务，不持久化为 rendered `.md` 上下文文件。implementer/reviewer 不在执行期调用 `wiki-researcher` 或重新选择 wiki；缺失或不足时回 planning 修复 sidecar。sourceOfTruth 不再有 task-scoped renderer；任务完成前改为对本任务真实 changed paths 运行 `source_truth_settings.py --lint-changed`，按 `block` / `ask` / `warn` 结果处理后才能标记完成。
- `subagent-driven-development`：dispatch 前先做一次 wiki fingerprint preflight；派发每个 implementer / reviewer 前，用 plugin-root `wiki_context_render.py --task-id <id> --role implementer|reviewer` 把该任务的 wiki 约束**渲染到文件**（与原生 `task-brief` / `review-package` 同一个工作树 SDD handoff 目录 `$(git rev-parse --show-toplevel)/.superpowers/sdd` 下的 `task-<id>-wiki.md` / `task-<id>-wiki-review.md`，落在工作树内而非 `.git/` 受保护路径），在 subagent prompt 里只给文件路径让其 Read，对齐 6.0.0 原生 `task-brief` / `review-package` 的 file-handoff 模型 —— 不再把完整任务文本（由原生 `task-brief` 负责）或渲染后的 wiki 约束粘进 prompt。hard-constraint rereads 也追加到同一任务 wiki 文件。implementer/reviewer subagent 只消费该 handoff 文件，不调用 `wiki-researcher`、不派发新的 wiki-selection subagent、不手动重读 wiki root；缺失或不足时通过 `BLOCKED` / `NEEDS_CONTEXT` 升级回 controller，让 planning 修复。task id 用 plan 的纯数字任务号（`### Task 1:`），与原生 `task-brief` 抽取一致。sourceOfTruth enforcement 由任务完成前的 changed-path lint 负责，不传递 source-truth sidecar。（一般嵌套派发约束已移除：6.0.0 原生 implementer/reviewer 通过 `BLOCKED` / `NEEDS_CONTEXT` 升级回 controller，已覆盖非 wiki 的上下文升级场景。）
- `finishing-a-development-branch`：在 `## The Process` 顶部、原生 Step 1（验证测试）之前注入一道 **durable-knowledge gate**。它不替代 Step 1，也不声称测试通过；它要求在执行任一收尾选项、worktree（及 `docs/superpowers/plans/<plan-stem>.wiki-candidates.jsonl` sidecar）还存活之前，确认本次工作的 `update-wiki` 去留判定已经做出。Option 1/4 会在 Step 6 删除 Superpowers 创建的 worktree，连带删掉该 sidecar，所以这是最后可靠的消费点。如果执行末尾已经为同一批工作跑过 `update-wiki`（正常路径），确认结论即可、无需重跑；否则现在就实跑 `update-wiki`：去留判定必须由实跑 skill 得出，不能在主循环里预判「无 durable knowledge」就绕过 skill——候选缺失（空/缺 sidecar 或 subagent 未报候选）不构成结论依据，因为 skill 还会权衡已实现改动与对话上下文。「跳过」是允许的结论，但必须是 skill 自己给出并带明确理由。该 gate 只做知识捕获，判定做出后不阻塞任何收尾选项。

当前流程不安装 SessionStart hook，但会安装一个 PostToolUse hook（matcher 为 `Bash`，脚本 `hooks/post-merge-update-wiki`）：当某次 Bash 命令把开发分支合并进其集成分支（裸 `git merge`、`git merge --continue` 或 `gh pr merge`，含 `git -C <dir> merge`）时，hook 注入一条 `update-wiki` 提醒，让主 agent 在收尾前审查是否产生 durable knowledge。它只针对「工作被接受/合并」这个动作，不依赖固定目标分支名——所以无论合回 `main` 还是迭代分支都生效；被跳过的只是「把主干/默认分支合进当前分支」的同步方向（存在 worktree origin metadata 时按 `originalBranch` 精确判向，否则按 `main`/`master`/默认分支启发式判断）、冲突未完成（存在 `MERGE_HEAD`）和 abort/非合并命令。它**不按本地 wiki 是否存在来 gate**：shared wiki 可能是全局配置的 MCP、本地零标记，任何文件系统判断都会漏，所以 finalize 合并一律触发，是否真要回写交给 `update-wiki` 自身 gate 决定（默认 skip，对项目 wiki、本地 shared、远程 MCP shared 都是 present 才读，无可写内容或无 wiki 时干净跳过）。durable knowledge 的**主消费点是执行末尾**：`executing-plans`/SDD 在实现（与 SDD 评审）完成后、移交 `finishing-a-development-branch` 或清理/退出 worktree **之前**，就在仍存活的 worktree 内调用 `update-wiki` 消费 `docs/superpowers/plans/<plan-stem>.wiki-candidates.jsonl`（该 sidecar 是 worktree 本地、随 worktree 删除而消失的 transient scratch，consume 后即删，从不提交）。因此**这条 post-merge 提醒只是最外层 backstop**：覆盖用户绕开 `finishing-a-development-branch` 直接合并、或漏跑执行末尾消费的情况。在两者之间还有一道中间闸：`finishing-a-development-branch` 自身在 `## The Process` 顶部带一道 durable-knowledge gate，在 Step 6 清理 worktree（连带删除 sidecar）之前再确认一次去留判定已由 `update-wiki` 实跑得出，防止主 agent 在主循环里预判「无候选」就直接进收尾、跳过 skill。三层（执行末尾主消费 → finishing gate → post-merge 提醒）都要求判定由实跑 `update-wiki` 得出，「跳过」必须是 skill 自己的结论。它触发时若主消费已为同一批工作跑过 `update-wiki`，确认结论即可、无需重跑；若彼时 worktree 已清、sidecar 已不在，则不应据此认定「无候选」，改从已合并 diff 与对话上下文做去留判定（`update-wiki` 默认 skip + 语义去重，重复触发不会重复落盘）。这条提醒不代表实现已通过验证，也无法捕获在 GitHub 网页上点击 Merge 的合并。`wiki-researcher` 会在 `brainstorming` 和 `writing-plans` 阶段按需读取 `.superpowers/wiki/` 和 `.shared-superpowers/wiki/`，并可在 `systematic-debugging` Phase 1 证据收窄后作为低噪音调试辅助被条件式调用。worktree origin metadata 是本地临时协调状态，不写入 `plan.md`、`spec.md`、`.superpowers/` 或仓库工作区。
...
| 4.6 | 可选 在独立 wiki 仓库内直接维护 | `./manage.sh export-wiki-skills /path/to/wiki-repo [--no-graph-ci]` 后在该仓库用 `update-wiki` / `migrate-wiki` skill | 拥有「仓库根即 wiki」的独立 wiki 仓库、想就地策展时 | 把 vendored 脚本闭包 + 两个 repo-local skill 钉版本写入仓库 `.claude/`（`.claude/wiki-tools/scripts/` + `.claude/skills/{update-wiki,migrate-wiki}/`）；运行时零依赖 adapter、经 `--wiki-dir` 把仓库根当 wiki 根；两个 skill 只改不提交（重建 `*.index.md`/目录 index auto-block/`.graph.json` + 中性与结构校验后停下，提交交给用户）。**默认还钉版本写入 `.github/workflows/rebuild-wiki-graph.yml`**：消费者 PR（含 4.5 的 shared-wiki MCP PR 路径）只带得动逐页 companion `*.index.md`，带不动跨页根 `.graph.json`（section 边 + backlinks），而 MCP 的 1 跳 `depends-on` 邻居正读这份文件，所以合入后到维护者手动重建前会陈旧；该 Action 在 push 到默认分支时复用**同一份** vendored `wiki_migrate_helper.py --generate-indexes` 重建根 `.graph.json` 并带 `[skip ci]` 回写（只在默认分支、no-diff 不提交，图生成器与维护者侧绝不分叉）。`--no-graph-ci` 跳过/移除该 Action（仅移除自己钉的 marker 文件，手写同名 workflow 不动）；默认分支若有保护规则挡住 `github-actions` bot 推送，改为允许该 bot 或退回手动重建。这是作者/策展侧，区别于 4.5 的消费者→PR 路径 |
...
当前 adapter 以 Superpowers 6.1.0 为适配基线，6.0.0 是硬性最低版本（`manifest.json` 的 `minSuperpowersVersion`）。低于 6.0.0 的 Superpowers 一律**跳过、不安装**：`./manage.sh` 的目标发现会过滤掉 sub-6.0.0 目标（若全部目标都低于基线则直接报错让用户升级或显式传目标），`install` 即便被显式指向 sub-6.0.0 目标也拒绝写入。安装时优先读取目标目录里的 `package.json` 版本号（自动发现时读 `installed_plugins.json` 记录）；同一 6.x 大版本内的补丁/小版本属基线内不再报警告，只有检测到更高**大版本**时才提示安装后重新校验。`superpowers@claude-plugins-official` 是当前自动发现安装目标的默认插件键；如果将来 Superpowers 改了这个安装记录键，显式传目标路径会更稳。上游 skill 标题和锚点如果变化，adapter 的 native patch 也需要同步检查。

高级配置：`adapter.config.json` 默认是 `{}`，不会改变任何 subagent 模型；adapter 只为自己的 overlay agent（`wiki-researcher`、Lanhu analyst）设模型，未设时保持 `model: inherit`。adapter 不再 patch Superpowers 上游 prompt template —— 6.0.0 起 implementer / task-reviewer 模板自带原生 `model:` 槽和 `## Model Selection` 段，要钉这些模型应在 plan/SDD 流程里填原生占位符，而不是通过 adapter。如果需要为 `wiki-researcher` 或 Lanhu analyst 指定模型，可参考 `adapter.config.example.jsonc`，把 `subagentModels.agents` 条目复制为无注释 JSON 后再运行 `./manage.sh install`；该项写入 adapter 原生 agent frontmatter，允许类似 `deepseek-v4-pro[1m]` 的 Claude Code 方括号后缀模型名，但 install 会对非 `inherit` / `sonnet` / `opus` / `haiku` 值提示 warning。`agents` 是 `subagentModels` 唯一支持的键，其它键（如已移除的 `upstreamPromptTemplates`）会导致 install 失败。
...
`.wiki-context.json` 是 schemaVersion 4 的 source of truth，应使用 page-rooted `wikiPages` 结构：每个 page 包含路径、root、source、`displayPath`、本地 `localPath` 或 MCP `wikiPath` / `revision`、来自伴随 `<stem>.index.md` 的有界 `documentContext`，以及嵌套 `sections`。每个 section 包含 `sectionId`（唯一 section id，不再写冗余 `section_name`）、`relevanceTo` 描述、hard constraint 标记、必要原文锚点、caveats、section-level `reread` metadata，以及 `implementation` / `test` / `review` / `general` 分类约束（空桶不写盘，只保留非空类别）；无法可靠分类但不能丢失的约束放入 `general`。执行期每 task 渲染只注入执行所需字段（relevanceTo / reason / hard constraint / 约束 / reread），不含 relevance / confidence / destination 等规划记账位。路由全部落在每段 section 的 `destination`（`kind` + `reason` + task-bound 的 `tasks`）由 planning agent 在 final task 稳定后一次性编辑（无 `globalWikiRefs`/`wikiRefs` 集合、无 `sectionRef` 手写、无 legacy `appliesTo`），随后由 `wiki_context_render.py <sidecar> --finalize` 一步机械补 `taskWikiRefs` roster、从 plan 任务文本 stamp `wiki/source task fingerprint`、校验执行就绪、单次写盘（不手写或复制；`--scaffold-tasks`/`--bind-fingerprints` 仍作为独立子命令保留，执行期换绑漂移时用 `--bind-fingerprints` 重新 stamp）。`documentContext` 只用于保留页面级主语和适用范围，不能包含 sibling sections 或整页正文；对于 `source: github_mcp`，`.shared-superpowers/wiki/<path>.md` 是逻辑展示路径而不是本地文件路径。如果 selected wiki page 与本次 Superpowers spec 冲突，应先让用户确认是调整需求 spec 还是更新项目 wiki，再写 plan。
...
`executing-plans` 和 `subagent-driven-development` 执行前应读取 plan 中的 `Referenced Project Wiki`，定位其中链接的 `.wiki-context.json`，再用 plugin-root `wiki_context_render.py` 先做一次 `wiki_context_render.py <sidecar> --fingerprint-preflight --execution-ready --strict --plan-path` 校验，再按 `--task-id <stable-task-id>` 渲染 selected role 约束块。主 agent / orchestrator 应捕获 renderer stdout，并在 prompt 中以 `## Rendered Wiki Constraints for This Task` 标注 source sidecar、task id、role 和 fingerprint-preflight 状态后直接注入；正常执行不应写入或传递 `.claude-*-wiki-task*-impl.md`、`.claude-*-source-task*-impl.md` 等 rendered task-context Markdown 文件。硬约束 section 的 forced reread 由单一固定取数器 `wiki_materialize_task.py --task-id <stable-task-id> --role <role> --execution-ready` 完成：它内部列出当前 task 的 selected hard constraints（与 `--reread-list` 同一选集），本地 section 直接从文件系统抽取，`source: github_mcp` section 调用 shared-wiki MCP 的 `read-sections` CLI（复用 server 同一份 `loadConfig` + `readSectionsTool`，全系统只有一份 shared-wiki 读取实现），并在 `## Hard Wiki Constraint Rereads` 下注入选中 section 全文（有界 document context 已由 renderer 每页渲染一次，reread 块不再重复），而不是补读整页 wiki；保持原始 reread-list 顺序、只含 selected sections，遇到 shared-wiki 换绑漂移（sidecar `sharedWiki.repoUrl` 与连接 repo 不一致）、revision 漂移、section 错误或 partial 结果一律 fail-closed。`--append-to` 把结果追加进 SDD handoff 文件（subagent 在同一文件 Read），省略则打到 stdout 供内联注入。renderer stdout 注入和 hard reread 是互补关系：前者是 planning 冻结后的结构化约束视图，后者是 hard section 原文权威回读。执行阶段不按 task string 模糊过滤，也不重新匹配 wiki；implementer/reviewer 不调用 `wiki-researcher`、不派发新的 wiki-selection subagent、不手动重读 wiki root。若 rendered constraints 或 hard rereads 缺失/明显不足，应暂停并回到 planning 修复 `Referenced Project Wiki` 和 `.wiki-context.json`；如果 plan 审核后被手工修改，先确认 routing 仍适用于改动的 task，再重新运行 `wiki_context_render.py <sidecar> --bind-fingerprints` 刷新 fingerprint，不重写 plan。
