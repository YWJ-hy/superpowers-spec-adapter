...
`.wiki-context.json` 是 schemaVersion 4 的 source of truth，应使用 page-rooted `wikiPages` 结构：每个 page 包含路径、root、source、`displayPath`、本地 `localPath` 或 MCP `wikiPath` / `revision`、来自伴随 `<stem>.index.md` 的有界 `documentContext`，以及嵌套 `sections`。每个 section 包含 `sectionId` / `section_name`、`relevanceTo` 描述、hard constraint 标记、必要原文锚点、caveats、section-level `reread` metadata，以及 `implementation` / `test` / `review` / `general` 分类约束；无法可靠分类但不能丢失的约束放入 `general`。`taskRouting` 与 `taskWikiRefs` roster 在 final task 稳定后由 `wiki_context_render.py <sidecar> --scaffold-tasks` 机械补脚手架；路由全部落在每段 section 的 `destination`（`kind` + `reason` + task-bound 的 `tasks`）由 planning agent 编辑（无 `globalWikiRefs`/`wikiRefs` 集合、无 `sectionRef` 手写、无 legacy `appliesTo`），`wiki/source task fingerprint` 则由 `wiki_context_render.py <sidecar> --bind-fingerprints` 从 plan 任务文本机械 stamp（不手写或复制）。`documentContext` 只用于保留页面级主语和适用范围，不能包含 sibling sections 或整页正文；对于 `source: github_mcp`，`.shared-superpowers/wiki/<path>.md` 是逻辑展示路径而不是本地文件路径。如果 selected wiki page 与本次 Superpowers spec 冲突，应先让用户确认是调整需求 spec 还是更新项目 wiki，再写 plan。
