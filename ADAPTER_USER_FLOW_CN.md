...
- `subagent-driven-development`：dispatch 前先做一次 wiki fingerprint preflight；派发每个 implementer / reviewer 前，用 plugin-root `wiki_context_render.py --task-id <id> --role implementer|reviewer` 把该任务的 wiki 约束**渲染到文件**（与原生 `task-brief` / `review-package` 同一个工作树 SDD handoff 目录 `$(git rev-parse --show-toplevel)/.superpowers/sdd` 下的 `task-<id>-wiki.md` / `task-<id>-wiki-review.md`，落在工作树内而非 `.git/` 受保护路径），在 subagent prompt 里只给文件路径让其 Read，对齐 6.0.0 原生 `task-brief` / `review-package` 的 file-handoff 模型 —— 不再把完整任务文本（由原生 `task-brief` 负责）或渲染后的 wiki 约束粘进 prompt。hard-constraint rereads 也追加到同一任务 wiki 文件。task id 用 plan 的纯数字任务号（`### Task 1:`），与原生 `task-brief` 抽取一致。sourceOfTruth enforcement 由任务完成前的 changed-path lint 负责，不传递 source-truth sidecar。（嵌套派发约束已移除：6.0.0 原生 implementer/reviewer 通过 `BLOCKED` / `NEEDS_CONTEXT` 升级回 controller，已覆盖该场景。）
