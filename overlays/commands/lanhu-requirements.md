...
3. Resolve Lanhu output settings from project-local `.superpowers/settings.json` before routing. Use `scripts/lanhu_settings.py <repo-root>` to inspect configured role and output format, or `scripts/lanhu_settings.py <role> <repo-root>` when role needs to be forced by the user; the settings resolver will surface configured role, if any, and `lanhu.frontend.output.format` defaults to `markdown`; allowed values are `markdown` and `html`; HTML is frontend-only; backend Markdown-only is mandatory.
