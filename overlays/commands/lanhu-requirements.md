...
When frontend output is configured as `html`, route to `lanhu-frontend-html-requirements-analyst` and write `index.html` as the complete HTML PRD main document plus a directoryized interaction prototype:
...
3. Resolve Lanhu output settings from project-local `.superpowers/settings.json` before routing. Use `scripts/lanhu_settings.py <role> <repo-root>` when available, or apply the same policy mechanically: `lanhu.frontend.output.format` defaults to `markdown`; allowed values are `markdown` and `html`; HTML is frontend-only; backend Markdown-only is mandatory.
...
- [ ] Frontend HTML output routed to `lanhu-frontend-html-requirements-analyst`, wrote package-root `index.html` as a complete HTML PRD main document plus `prototype/index.html` as the directoryized interaction prototype, or returned the text-only Markdown fallback metadata.
