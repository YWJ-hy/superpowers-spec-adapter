...
16. Do not overwrite existing files or directories:
   ...
   - Keep all generated PRD files inside the package directory, including `.lanhu/MM-DD-需求名称/prds/` when the Markdown scope splits into multiple delivery boundaries.
...
18. Only after user confirmation, continue with Superpowers brainstorming using that package as the requirements description. `index.md` is the entry point and the generated PRD artifact files are the detailed requirements sources. There are no design links or UI reference files.
