...
This file is generated from `overlays/agents/lanhu-requirements-analyst.common.md` and `role-prd/frontend.md` and `role-prd/frontend_outputHtml.md`. Edit those sources, then run `./manage.sh install` or `python3 lib/sync_role_prd.py sync`.
...
<!-- superpower-adapter:auxiliary-output-templates:start role=frontend -->

### frontend HTML output auxiliary output template

Generated verbatim from `role-prd/frontend_outputHtml.md`. Apply this auxiliary output template only when frontend role only when outputPreference.format is markdown+html and outputPreference.htmlPrototype.enabled is true.
...
<!-- superpower-adapter:auxiliary-output-templates:end role=frontend -->
