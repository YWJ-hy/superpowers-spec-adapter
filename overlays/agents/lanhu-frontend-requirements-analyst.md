...
Primary artifact: index.md plus role-prd/prd.md plus optional role-prd/design/index.html and role-prd/design/assets/
Fallback artifact: not applicable; frontend always writes role-prd/prd.md, and the design demo is optional
...
For `outputMode: package`, write package files directly and return compact metadata only. `packageDir`, `indexPath`, and `writtenFiles` must point inside the selected `.lanhu/MM-DD-需求名称/` directory. `index.md` is the entry index and must include `证据包角色：frontend` and `包类型：frontend_unified`.
