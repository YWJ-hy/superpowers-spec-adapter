...
from spec_common import build_spec_index_graph, rel_posix, repo_root
...
def check_leaf(spec_root: Path, path: Path) -> tuple[list[str], list[str]]:
    warnings: list[str] = []
    errors: list[str] = []
    rel = rel_posix(spec_root.resolve(), path)
    try:
    ...
    if not text.strip():
        warnings.append(f"Empty indexed spec: {rel}")
    if "<fill-me>" in text:
...
def check_spec_tree() -> dict:
    ...
    for leaf in graph.leaves:
        leaf_warnings, leaf_errors = check_leaf(spec_root, leaf)
        warnings.extend(leaf_warnings)
        errors.extend(leaf_errors)

    if not graph.leaves:
        warnings.append("No indexed leaf specs found")
...
def render_text(result: dict) -> str:
    ...
    heading = {
        VALID: "SPEC_UPDATE_CHECK_VALID: spec index and leaf mechanics look valid",
        WARNING: "SPEC_UPDATE_CHECK_WARN: mechanical warnings found",
        INVALID: "SPEC_UPDATE_CHECK_INVALID: mechanical errors found",
    }[status]
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Validate .superpowers/spec index and leaf mechanics.")
