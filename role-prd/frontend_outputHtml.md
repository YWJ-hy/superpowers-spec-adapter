...
- 当本 analyst 被主流程以 `pagePackageMode: true` 调用来处理多页面蓝湖运行中的单个页面时，仍必须产出完整原风格前端 HTML PRD 页面包：该页面包的 `index.md` 是页面包入口，`index.html` 是该页面完整前端 PRD 主文档，`prototype/index.html` 是该页面 1:1 交互/控件核对原型。页面包可以引用 sibling 页面作为关系上下文，但不能依赖聚合 `index.md` 才能理解自身需求，也不得因为“主 agent 稍后会汇总”而省略 PRD 章节、字段规则、状态、交互、验收或待确认问题。
